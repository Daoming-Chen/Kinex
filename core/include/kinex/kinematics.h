...
namespace kinex
{
    ...
    {
        std::vector<Eigen::Isometry3d> link_transforms; ///< Global transform for each link
        std::vector<Eigen::Vector3d> joint_positions;   ///< Position of each joint in world frame
        std::vector<Eigen::Vector3d> joint_axes;        ///< Axis of each joint in world frame
    ...
    public:
        ...
        /**
         * @brief Compute forward kinematics and populate RobotState for FK/Jacobian fusion
         * @param joint_angles Vector of joint angles
         * @param state Output parameter - populated with link transforms and joint data
         * @param check_bounds If true, verify joint angles are within limits
         * @return Transform from base to end-effector
         */
        Transform computeWithState(
            const Eigen::VectorXd &joint_angles,
            RobotState &state,
            bool check_bounds = false) const;

        /**
         * @brief Compute forward kinematics and Jacobian together (fusion for efficiency)
         * @param joint_angles Vector of joint angles
         * @param state Output parameter - populated with link transforms and joint data
         ...
         * @return Transform from base to end-effector
         */
        Transform computeWithStateAndJacobian(
            const Eigen::VectorXd &joint_angles,
            RobotState &state,
    ...
    public:
         ...
         * @param state Pre-computed RobotState from ForwardKinematics::computeWithState
         ...
         */
        Eigen::MatrixXd computeFromState(
            const Eigen::VectorXd &joint_angles,
