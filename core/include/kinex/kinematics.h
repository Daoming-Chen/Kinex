...
/**
 * @brief Cache of global link transforms for efficient FK/Jacobian fusion
 *
 * RobotState stores pre-computed global transforms for all links in a chain,
 * allowing FK and Jacobian calculations to reuse the same transform data
 * without recomputation, eliminating redundant FK evaluations.
 */
struct KINEX_API RobotState {
  std::vector<Eigen::Isometry3d>
      link_transforms; ///< Global transform for each link
  std::vector<Eigen::Vector3d>
      joint_positions; ///< Position of each joint in world frame
  std::vector<Eigen::Vector3d>
      joint_axes; ///< Axis of each joint in world frame

  void resize(size_t num_links, size_t num_joints) {
    link_transforms.resize(num_links);
    joint_positions.resize(num_joints);
    joint_axes.resize(num_joints);
  }
...
public:
   ...
   */
  KinematicChain(std::shared_ptr<const RobotModel> robot,
   ...
   */
  const std::vector<Transform> &getStaticTransforms() const {
    return static_transforms_;
  }

  /**
   * @brief Get all joints in the chain (including fixed joints)
   * This is used for FK computation which needs to include fixed joint
   * transforms
   */
  const std::vector<std::shared_ptr<Joint>> &getAllJoints() const {
    return all_joints_;
  }
...
private:
   ...
   */
  std::vector<std::shared_ptr<Joint>> findPath(const std::string &from_link,
...
public:
   ...
   * @param base_link Name of the base link (defaults to robot root)
   */
  ForwardKinematics(std::shared_ptr<const RobotModel> robot,
                    const std::string &end_link,
                    const std::string &base_link = "");

  /**
   * @brief Compute forward kinematics for given joint angles
   * @param joint_angles Vector of joint angles (size must match number of
   * joints)
   ...
   */
  Transform compute(const Eigen::VectorXd &joint_angles,
                    bool check_bounds = false) const;

  /**
   * @brief Compute forward kinematics and populate RobotState for FK/Jacobian
   * fusion
   * @param joint_angles Vector of joint angles
   * @param state Output parameter - populated with link transforms and joint
   * data
   * @param check_bounds If true, verify joint angles are within limits
   * @return Transform from base to end-effector
   */
  Transform computeWithState(const Eigen::VectorXd &joint_angles,
                             RobotState &state,
                             bool check_bounds = false) const;

  /**
   * @brief Compute forward kinematics and Jacobian together (fusion for
   * efficiency)
   * @param joint_angles Vector of joint angles
   * @param state Output parameter - populated with link transforms and joint
   * data
   * @param jacobian Output parameter - 6xN Jacobian matrix
   * @param check_bounds If true, verify joint angles are within limits
   * @return Transform from base to end-effector
   */
  Transform computeWithStateAndJacobian(const Eigen::VectorXd &joint_angles,
                                        RobotState &state,
   ...
   */
  Transform computeToLink(const Eigen::VectorXd &joint_angles,
  ...
  computeAllLinkTransforms(const Eigen::VectorXd &joint_angles,
                           bool check_bounds = false) const;
...
private:
   ...
   */
  Transform computeToJointIndex(const Eigen::VectorXd &joint_angles,
...
public:
  JacobianCalculator(std::shared_ptr<const RobotModel> robot,
                     const std::string &end_link,
                     const std::string &base_link = "");
  ...
  Eigen::MatrixXd compute(const Eigen::VectorXd &joint_angles,
                          JacobianType type = JacobianType::Analytic,
                          const std::string &target_link = "") const;

  /**
   * @brief Compute Jacobian from pre-computed RobotState (FK/Jacobian fusion)
   * @param joint_angles Vector of joint angles (needed for verification)
   * @param state Pre-computed RobotState from
   * ForwardKinematics::computeWithState
   * @param type Type of Jacobian to compute
   * @param target_link Target link name (empty for end effector)
   * @return 6xN Jacobian matrix
   */
  Eigen::MatrixXd computeFromState(const Eigen::VectorXd &joint_angles,
                                   const RobotState &state,
                                   JacobianType type = JacobianType::Analytic,
                                   const std::string &target_link = "") const;

  Eigen::MatrixXd
  computeJacobianDerivative(const Eigen::VectorXd &joint_angles,
                            const Eigen::VectorXd &joint_velocities,
                            JacobianType type = JacobianType::Analytic,
                            const std::string &target_link = "") const;

  bool isSingular(const Eigen::VectorXd &joint_angles, double threshold = 1e-6,
                  JacobianType type = JacobianType::Analytic,
                  const std::string &target_link = "") const;

  double getManipulability(const Eigen::VectorXd &joint_angles,
                           JacobianType type = JacobianType::Analytic,
                           const std::string &target_link = "") const;

  double getConditionNumber(const Eigen::VectorXd &joint_angles,
                            JacobianType type = JacobianType::Analytic,
                            const std::string &target_link = "") const;

  static Eigen::MatrixXd convertJacobian(const Eigen::MatrixXd &jacobian,
                                         const Transform &pose,
                                         JacobianType from, JacobianType to);
...
private:
  ...
  void computeAnalyticalJacobian(const Eigen::VectorXd &joint_angles,
  ...
  mutable std::unordered_map<std::string, std::unique_ptr<KinematicChain>>
      chain_cache_;
  mutable std::unordered_map<std::string, std::unique_ptr<ForwardKinematics>>
      fk_cache_;
