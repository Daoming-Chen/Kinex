...
public:
   ...
   */
  static Transform fromPositionQuaternion(const Eigen::Vector3d &position,
   ...
   */
  static Transform fromPositionRPY(const Eigen::Vector3d &position,
...
public:
  Joint(const std::string &name, JointType type) : name_(name), type_(type) {}
   ...
   */
  bool isActuated() const { return type_ != JointType::Fixed; }
...
public:
  ...
  getChildJoints(const std::string &link_name) const;
