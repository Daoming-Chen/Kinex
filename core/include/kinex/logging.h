 ...
 *
 * @param level_str Log level string (trace, debug, info, warn, error, critical,
 * off)
 */
...
inline void setLogFile(const std::string &filepath, bool truncate = false) {
  try {
    auto file_sink =
        std::make_shared<spdlog::sinks::basic_file_sink_mt>(filepath, truncate);
  ...
  sinks.erase(
      std::remove_if(sinks.begin(), sinks.end(),
                     [](const spdlog::sink_ptr &sink) {
                       return dynamic_cast<spdlog::sinks::basic_file_sink_mt *>(
                                  sink.get()) != nullptr;
                     }),
      sinks.end());
}
