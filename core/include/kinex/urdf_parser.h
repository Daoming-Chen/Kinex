...
private:
  ...
  static std::string formatMessage(const std::string &message, int line) {
    if (line >= 0) {
      return "URDF parse error at line " + std::to_string(line) + ": " +
             message;
    }
...
public:
   ...
   */
  void setBaseDirectory(const std::string &base_dir) { base_dir_ = base_dir; }

  const std::string &getBaseDirectory() const { return base_dir_; }
...
private:
  ...
  std::shared_ptr<RobotModel> parseDocument(const std::string &content,
