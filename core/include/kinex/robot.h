...
public:
  ...
  // Constructors
  Robot(std::shared_ptr<const RobotModel> model, const std::string &end_link,
        const std::string &base_link = "");
  ...
  Transform computePose(const Eigen::VectorXd &q,
                        const std::string &link = "") const; // Alias

  /**
   * @brief Compute transforms for all links in the robot (visual/collision
   * update)
   */
  std::unordered_map<std::string, Transform>
  computeAllLinkTransforms(const Eigen::VectorXd &q) const;
  ...
  std::pair<Eigen::VectorXd, SolverStatus>
  solveIK(const Transform &target, const Eigen::VectorXd &q_init,
          const std::string &link = ""); // Alias

  // Jacobian
  Eigen::MatrixXd
  computeJacobian(const Eigen::VectorXd &q, const std::string &link = "",
                  JacobianType type = JacobianType::Analytic) const;

  double getManipulability(const Eigen::VectorXd &q,
                           const std::string &link = "") const;
  bool isSingular(const Eigen::VectorXd &q, double threshold = 1e-6,
                  const std::string &link = "") const;
...
private:
  ...
  mutable std::unordered_map<std::string, std::unique_ptr<JacobianCalculator>>
      jacobian_cache_;
