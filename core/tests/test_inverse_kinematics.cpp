...
Transform makeTransform(const Eigen::Vector3d &position,
  ...
  Transform result =
      Transform::fromPositionQuaternion(position, quaternion.normalized());
  return result;
}
...
protected:
  ...
  Eigen::VectorXd randomConfiguration(double scale = 0.8) {
    ...
    for (size_t i = 0; i < dof_; ++i) {
      double mid = 0.5 * (upper_limits_[static_cast<Eigen::Index>(i)] +
                          lower_limits_[static_cast<Eigen::Index>(i)]);
      double span = 0.5 * (upper_limits_[static_cast<Eigen::Index>(i)] -
                           lower_limits_[static_cast<Eigen::Index>(i)]);
...
TEST_F(InverseKinematicsTest, WarmStartReducesIterations) {
  ...
  EXPECT_LT((fk_->compute(warm).translation() - target.translation()).norm(),
            1e-4);
}
...
TEST_F(InverseKinematicsTest, RespectsJointLimits) {
  ...
  for (size_t i = 0; i < dof_; ++i) {
    EXPECT_LE(result[static_cast<Eigen::Index>(i)],
              upper_limits_[static_cast<Eigen::Index>(i)] + 1e-3);
    EXPECT_GE(result[static_cast<Eigen::Index>(i)],
              lower_limits_[static_cast<Eigen::Index>(i)] - 1e-3);
  }
}
...
TEST_F(InverseKinematicsTest, PositionOnlyIgnoresOrientationDifferences) {
  ...
  rotated.linear() = Eigen::AngleAxisd(M_PI / 3.0, Eigen::Vector3d::UnitX())
                         .toRotationMatrix() *
                     rotated.linear();
...
TEST_F(InverseKinematicsTest,
       OrientationOnlyAdjustsOrientationWithMinimalTranslation) {
  SQPIKSolver solver(robot_, end_link_, base_link_);
  auto config = solver.getSolverConfig();
  ...
  rotated.linear() =
      rotated.linear() *
      Eigen::AngleAxisd(kRotation, Eigen::Vector3d::UnitZ()).toRotationMatrix();
  Transform target(rotated);
...
TEST_F(InverseKinematicsTest, MultipleInitialGuessesFindDifferentSolutions) {
  ...
  EXPECT_LT((fk_->compute(first).translation() - target.translation()).norm(),
            1e-4);
  EXPECT_LT((fk_->compute(second).translation() - target.translation()).norm(),
            1e-4);
...
TEST_F(InverseKinematicsTest, ConvergesForRandomTargets) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    EXPECT_TRUE(status.converged) << "Failed on iteration " << i
                                  << " with error: " << status.final_error_norm;
