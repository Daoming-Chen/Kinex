...
protected:
  void SetUp() override {
    ...
    // Get the UR5e URDF path - now in examples/models/ur5
...
<robot>
  ...
  URDFParser parser;
  EXPECT_THROW({ parser.parseString(bad_urdf); }, URDFParseException);
}
...
TEST_F(URDFParserTest, ErrorNonExistentFile) {
  URDFParser parser;
  EXPECT_THROW({ parser.parseFile("/non/existent/path.urdf"); },
               URDFParseException);
}
...
TEST_F(URDFParserTest, TransformComposition) {
  Transform t1 = Transform::fromPositionRPY(Eigen::Vector3d(1, 0, 0),
                                            Eigen::Vector3d(0, 0, 0));
  Transform t2 = Transform::fromPositionRPY(Eigen::Vector3d(0, 1, 0),
                                            Eigen::Vector3d(0, 0, 0));
...
TEST_F(URDFParserTest, TransformInverse) {
  Transform t = Transform::fromPositionRPY(Eigen::Vector3d(1, 2, 3),
