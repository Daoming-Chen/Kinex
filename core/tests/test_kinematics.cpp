...
Eigen::MatrixXd finiteDifferenceJacobian(const ForwardKinematics &fk,
  ...
  for (size_t i = 0; i < dof; ++i) {
    ...
    Eigen::Vector3d dp =
        (T_plus.translation() - T_minus.translation()) / (2.0 * epsilon);
    Eigen::Matrix3d dR =
        (T_plus.rotation() - T_minus.rotation()) / (2.0 * epsilon);
...
TEST_F(ForwardKinematicsTest, KinematicChainInvalidLinks) {
  ...
  EXPECT_THROW(KinematicChain(simple_robot_, "nonexistent_link", "base_link"),
               std::invalid_argument);

  // Test with non-existent base link
  EXPECT_THROW(KinematicChain(simple_robot_, "link2", "nonexistent_link"),
               std::invalid_argument);
}
...
TEST_F(ForwardKinematicsTest, ComputeWithBothJointsRotated) {
  ...
  // Position should be approximately (0, 1, 0) as both rotations cancel the
  // displacement
  Eigen::Vector3d position = result.translation();
  EXPECT_NEAR(position.x(), 0.0, 1e-6);
  EXPECT_NEAR(position.y(), 1.0, 1e-6);
...
TEST_F(ForwardKinematicsTest, ComputeToInvalidLink) {
  ...
  EXPECT_THROW(fk.computeToLink(joint_angles, "nonexistent_link"),
               std::invalid_argument);
}
...
TEST_F(ForwardKinematicsTest, UR5eRobotAtZero) {
  ...
  std::vector<std::string> possible_end_links = {"tool0", "ee_link",
                                                 "wrist_3_link"};
  std::string end_link;
...
TEST_F(ForwardKinematicsTest, PerformanceBenchmark) {
  ...
  EXPECT_LT(avg_time_ms, 1.0)
      << "Average FK computation time: " << avg_time_ms << " ms";

  // Log performance info
  std::cout << "FK Performance: " << avg_time_us << " us per computation"
            << std::endl;
}
...
TEST_F(ForwardKinematicsTest, ThreadSafety) {
  ...
  EXPECT_EQ(error_count, 0)
      << "Thread safety test failed with " << error_count << " errors";
}
...
TEST_F(ForwardKinematicsTest, JacobianDerivativeMatchesFiniteDifference) {
  ...
  EXPECT_THROW({ jc.computeJacobianDerivative(q, dq, JacobianType::Analytic); },
               std::runtime_error);
}
...
TEST_F(ForwardKinematicsTest, JacobianToIntermediateLink) {
  ...
  Eigen::Vector3d delta =
      fk_end.compute(q).translation() - fk_mid.compute(q_mid).translation();

  ASSERT_EQ(J_mid.cols(), 1);
  EXPECT_TRUE(
      (J_mid.block<3, 1>(3, 0).isApprox(J_end.block<3, 1>(3, 0), 1e-9)));

  Eigen::Vector3d shifted_linear =
      J_mid.block<3, 1>(0, 0) + J_mid.block<3, 1>(3, 0).cross(delta);
