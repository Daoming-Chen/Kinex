...
namespace kinex
{
    ...
    IKSolver::IKSolver(
        ...
        : robot_(std::move(robot)), end_link_(std::move(end_link)), base_link_(std::move(base_link))
    {
        if (!robot_)
        {
    ...
    SQPIKSolver::SQPIKSolver(
        ...
        : IKSolver(std::move(robot), end_link, base_link), fk_(robot_, end_link_, base_link_), jacobian_(robot_, end_link_, base_link_), qp_solver_(std::make_unique<DaQPSolver>(fk_.getNumJoints()))
    {
        ...
        Eigen::VectorXd &solution)
    {
        ...
        for (size_t iter = 0; iter < config_.max_iterations; ++iter)
        {
            ...
            Transform current_pose = fk_.computeWithStateAndJacobian(q_current_, robot_state_, J_);

            // Build and weight error (reuse pre-allocated arrays)
            weighted_error_.noalias() = weightError(buildTaskError(current_pose, target_pose));
            ...
            bool qp_ok = qp_solver_ && qp_solver_->solve(H_, g_, lower_delta_, upper_delta_, delta_, status.qp_status);
            if (!qp_ok)
            {
        ...
        if (!status.converged)
        {
            ...
            // Compute Jacobian for failure analysis (we can reuse the last computed one)
            Eigen::JacobiSVD<Eigen::MatrixXd> svd(J_, Eigen::ComputeThinU | Eigen::ComputeThinV);
            const auto &singular_values = svd.singularValues();
            double cond_number = singular_values(0) / singular_values(singular_values.size() - 1);
