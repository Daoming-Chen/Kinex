...
std::string getAttribute(const pugi::xml_node &node, const char *name,
                         const std::string &default_value = "") {
  pugi::xml_attribute attr = node.attribute(name);
  return attr ? attr.value() : default_value;
}

double getDoubleAttribute(const pugi::xml_node &node, const char *name,
...
Geometry parseGeometry(const pugi::xml_node &geometry_node,
  ...
  if (mesh) {
    ...
    if (!base_dir.empty() &&
        geometry.mesh_filename.find("://") == std::string::npos) {
...
Inertial parseInertial(const pugi::xml_node &inertial_node) {
  ...
  if (inertia_node) {
    ...
    inertial.inertia << ixx, ixy, ixz, ixy, iyy, iyz, ixz, iyz, izz;
  }

  return inertial;
}

Visual parseVisual(const pugi::xml_node &visual_node,
                   const std::string &base_dir) {
  Visual visual;
...
Collision parseCollision(const pugi::xml_node &collision_node,
                         const std::string &base_dir) {
  Collision collision;
...
public:
  std::shared_ptr<RobotModel> parse(const std::string &content,
    ...
    if (!result) {
      throw URDFParseException(std::string("XML parse error: ") +
                                   result.description(),
...
private:
  void parseLink(const pugi::xml_node &link_node,
    ...
    if (inertial_node) {
      try {
        ...
        throw URDFParseException("Error parsing inertial for link '" +
                                 link_name + "': " + e.what());
      }
    }
    ...
    for (pugi::xml_node visual_node : link_node.children("visual")) {
      try {
        ...
        throw URDFParseException("Error parsing visual for link '" + link_name +
                                 "': " + e.what());
      }
    }
    ...
    for (pugi::xml_node collision_node : link_node.children("collision")) {
      try {
        ...
        throw URDFParseException("Error parsing collision for link '" +
                                 link_name + "': " + e.what());
      }
    }
  ...
  void parseJoint(const pugi::xml_node &joint_node,
                  std::shared_ptr<RobotModel> robot) {
    ...
    if (type_str.empty()) {
      throw URDFParseException("Joint '" + joint_name +
    ...
    } else if (joint_type == JointType::Revolute ||
               joint_type == JointType::Prismatic) {
      KINEX_WARN("Joint '{}' of type '{}' missing <limit> element", joint_name,
                 type_str);
    }
...
URDFParser::parseString(const std::string &urdf_string) {
  KINEX_INFO("Parsing URDF from string ({} bytes)", urdf_string.size());
  return parseDocument(urdf_string, "<string>");
}

std::shared_ptr<RobotModel>
URDFParser::parseDocument(const std::string &content,
