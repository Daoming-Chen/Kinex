...
KinematicChain::KinematicChain(std::shared_ptr<const RobotModel> robot,
  ...
  KINEX_LOG_INFO("Building kinematic chain from '{}' to '{}'", base_link_,
                 end_link_);
  buildChain();
  KINEX_LOG_INFO("Kinematic chain built with {} actuated joints",
                 joints_.size());
}
...
void KinematicChain::buildChain() {
  ...
  while (current_link != base_link_) {
    ...
    if (!parent_joint) {
      throw std::runtime_error("No path found from '" + end_link_ + "' to '" +
                               base_link_ + "': link '" + current_link +
                               "' has no parent joint");
    }
  ...
  KINEX_LOG_DEBUG("Chain has {} total joints, {} actuated", path_joints.size(),
                  joints_.size());
}

std::vector<std::shared_ptr<Joint>>
KinematicChain::findPath(const std::string &from_link,
...
ForwardKinematics::ForwardKinematics(std::shared_ptr<const RobotModel> robot,
                                     const std::string &end_link,
                                     const std::string &base_link)
...
Transform ForwardKinematics::compute(const Eigen::VectorXd &joint_angles,
                                     bool check_bounds) const {
  if (static_cast<size_t>(joint_angles.size()) != chain_.getNumJoints()) {
    throw std::invalid_argument("Joint angles size (" +
...
Transform
ForwardKinematics::computeWithState(const Eigen::VectorXd &joint_angles,
                                    RobotState &state,
                                    bool check_bounds) const {
  if (static_cast<size_t>(joint_angles.size()) != chain_.getNumJoints()) {
    throw std::invalid_argument("Joint angles size (" +
...
Transform ForwardKinematics::computeWithStateAndJacobian(
    const Eigen::VectorXd &joint_angles, RobotState &state,
    Eigen::MatrixXd &jacobian, bool check_bounds) const {
  ...
  for (size_t i = 0; i < dof; ++i) {
    ...
    if (joint->getType() == JointType::Prismatic) {
      ...
      // Revolute: Linear velocity is z_i x (p_ee - p_i), Angular velocity is
      // z_i
...
Transform ForwardKinematics::computeToLink(const Eigen::VectorXd &joint_angles,
  ...
  if (it == link_names.end()) {
    throw std::invalid_argument("Target link '" + target_link +
                                "' not found in kinematic chain");
  }
...
Transform
ForwardKinematics::computeToJointIndex(const Eigen::VectorXd &joint_angles,
...
void ForwardKinematics::checkJointLimits(
  ...
  for (size_t i = 0; i < joints.size(); ++i) {
    ...
    if (limits && joint->getType() == JointType::Revolute) {
      ...
      if (q < limits->lower || q > limits->upper) {
        throw std::runtime_error("Joint '" + joint->getName() + "' value " +
...
ForwardKinematics::computeAllLinkTransforms(const Eigen::VectorXd &joint_angles,
                                            bool check_bounds) const {
  if (check_bounds) {
    checkJointLimits(joint_angles);
  }
  ...
  while (!to_visit.empty()) {
    ...
    const Eigen::Isometry3d &parent_transform =
        result[current_link].getTransform();
...
JacobianCalculator::JacobianCalculator(std::shared_ptr<const RobotModel> robot,
                                       const std::string &end_link,
                                       const std::string &base_link)
    : robot_(std::move(robot)),
      base_link_(base_link.empty() && robot_ ? robot_->getRootLink()
                                             : base_link),
      default_end_link_(end_link) {
  if (!robot_) {
    throw std::invalid_argument(
        "Robot pointer cannot be null for JacobianCalculator");
  }
  ...
  if (!robot_->getLink(default_end_link_)) {
    throw std::invalid_argument("End link not found in robot model: " +
                                default_end_link_);
  }

  if (!robot_->getLink(base_link_)) {
    throw std::invalid_argument("Base link not found in robot model: " +
                                base_link_);
  }
}

std::string
JacobianCalculator::resolveLink(const std::string &target_link) const {
  return target_link.empty() ? default_end_link_ : target_link;
}

const KinematicChain &
JacobianCalculator::ensureChain(const std::string &link) const {
...
JacobianCalculator::JointFrameCache &
JacobianCalculator::ensureCache(const std::string &link, size_t dof) const {
...
Eigen::MatrixXd
JacobianCalculator::compute(const Eigen::VectorXd &joint_angles,
                            JacobianType type,
  ...
  if (static_cast<size_t>(joint_angles.size()) < dof) {
    throw std::invalid_argument(
        "Joint vector size is smaller than chain DOF for Jacobian computation");
  }
  ...
  if (type == JacobianType::Analytic) {
    ...
    return convertJacobian(J, pose, JacobianType::Analytic,
                           JacobianType::Geometric);
  }
}

Eigen::MatrixXd
JacobianCalculator::computeFromState(const Eigen::VectorXd &joint_angles,
                                     const RobotState &state, JacobianType type,
                                     const std::string &target_link) const {
  const std::string link = resolveLink(target_link);
  const auto &chain = ensureChain(link);
  const size_t dof = chain.getNumJoints();
  ...
  if (state.joint_axes.size() < dof || state.joint_positions.size() < dof) {
    throw std::invalid_argument(
        "RobotState not properly initialized for Jacobian computation");
  }
...
Eigen::MatrixXd JacobianCalculator::computeJacobianDerivative(
    ...
    const Eigen::VectorXd &joint_velocities, JacobianType type,
    const std::string &target_link) const {
  (void)joint_angles;
  ...
  throw std::runtime_error(
      "computeJacobianDerivative not implemented for Analytical Jacobian");
}

void JacobianCalculator::computeAnalyticalJacobian(
    const Eigen::VectorXd &joint_angles, const KinematicChain &chain,
    JointFrameCache &cache, Eigen::MatrixXd &J_out) const {
  ...
  for (size_t i = 0; i < all_joints.size(); ++i) {
    ...
    if (joint->isActuated() && actuated_idx < dof) {
      ...
      // Cache joint axis and position in world frame BEFORE applying joint
      ...
      case JointType::Continuous: {
        ...
        if (axis_local.x() == 0 && axis_local.y() == 0 && axis_local.z() == 1) {
          double c = std::cos(q);
          double s = std::sin(q);
          joint_transform.linear() << c, -s, 0, s, c, 0, 0, 0, 1;
        } else {
    ...
    // Fixed joints only contribute their origin transform (already applied
    // above)
  }
...
bool JacobianCalculator::isSingular(const Eigen::VectorXd &joint_angles,
                                    double threshold, JacobianType type,
                                    const std::string &target_link) const {
...
double
JacobianCalculator::getManipulability(const Eigen::VectorXd &joint_angles,
                                      JacobianType type,
                                      const std::string &target_link) const {
  Eigen::MatrixXd J = compute(joint_angles, type, target_link);
  Eigen::MatrixXd gram;
...
double
JacobianCalculator::getConditionNumber(const Eigen::VectorXd &joint_angles,
                                       JacobianType type,
                                       const std::string &target_link) const {
  Eigen::MatrixXd J = compute(joint_angles, type, target_link);
  Eigen::JacobiSVD<Eigen::MatrixXd> svd(J, Eigen::ComputeThinU |
...
Eigen::MatrixXd
JacobianCalculator::convertJacobian(const Eigen::MatrixXd &jacobian,
                                    const Transform &pose, JacobianType from,
                                    JacobianType to) {
  if (from == to) {
    return jacobian;
  }
