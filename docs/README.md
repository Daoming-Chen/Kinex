# Kinex Documentation

Welcome to the Kinex documentation! This page provides an overview of all available documentation resources.

[中文文档](zh/) | [Back to Main README](../README.md)

## Getting Started

New to Kinex? Start here:

- **[Getting Started Guide](guides/getting-started.md)** - Installation and first steps
- **[Building from Source](guides/building.md)** - Compile Kinex for your platform
- **[Quick Examples](../examples/)** - Sample code to get you started

## API Reference

Detailed API documentation for each language binding:

- **[C++ API](api/cpp.md)** - Core library documentation with unified Robot class
- **[Python API](api/python.md)** - Python bindings reference with Robot class
- **WebAssembly API** - JavaScript/TypeScript bindings documentation (in development)

## Tutorials

Step-by-step guides for common tasks:

- **[C++ Tutorial](tutorials/cpp-tutorial.md)** - Using Kinex unified Robot API in C++ applications
- **Python Tutorial** - Python examples and patterns with Robot class (coming soon)
- **Web Application Tutorial** - Building web apps with WASM (coming soon)

## Performance

Understanding Kinex's performance characteristics:

- **[Benchmarks](../benchmarks/README.md)** - Performance analysis and benchmarking guide
- **[Mixed Chain Benchmarks](benchmarks/mixkinbench.md)** - Complex robot configurations

## Advanced Topics

- **Architecture Overview** (coming soon)
- **Algorithm Details** (coming soon)
- **Extending Kinex** (coming soon)

## Language-Specific Documentation

### Python

- [Getting Started with Python](guides/getting-started.md#python)
- [Python API Reference](api/python.md)
- [Python Examples](../examples/python/)

### JavaScript/WebAssembly

- [Getting Started with WASM](guides/getting-started.md#javascripttypescript)
- [Interactive Demo](https://daoming-chen.github.io/Kinex/)
- [JavaScript Examples](../examples/javascript/)

### C++

- [Getting Started with C++](guides/getting-started.md#c)
- [C++ Tutorial](tutorials/cpp-tutorial.md)
- [Building from Source](guides/building.md)
- [C++ Examples](../examples/cpp/)

## Contributing

Information for contributors:

- **[Contributing Guide](../CONTRIBUTING.md)** (coming soon)
- **[Code Style Guide](../CONTRIBUTING.md)** (coming soon)
- **[Development Setup](guides/building.md)**

## Support

Need help?

- 🐛 [Report Issues](https://github.com/Daoming-Chen/Kinex/issues)
- 💬 [Discussions](https://github.com/Daoming-Chen/Kinex/discussions)
- 📖 Browse this documentation
- 🌟 Check out the [examples](../examples/)

## Version History

- **v0.1.0** - Initial release (coming soon)

---

**Quick Links:**
- [Main README](../README.md)
- [GitHub Repository](https://github.com/Daoming-Chen/Kinex)
- [npm Package](https://www.npmjs.com/package/@daoming.chen/kinex)
- [PyPI Package](https://pypi.org/project/kinex/) (coming soon)
