...
def test_transform_from_pos_quat():
    ...
    t = kinex.Transform.from_position_quaternion(pos, quat)
...
def test_transform_rpy():
    ...
    t = kinex.Transform.from_position_rpy(pos, rpy)
...
def test_inverse():
    t = kinex.Transform.from_position_rpy(np.array([1.0, 0.0, 0.0]), np.array([0.0, 0.0, 0.0]))
    inv = t.inverse()
