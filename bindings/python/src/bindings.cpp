...
NB_MODULE(_kinex, m) {
  ...
  nb::class_<Joint>(m, "Joint")
      ...
      .def("get_transform", &Joint::getTransform,
           "Get transformation for given joint value");
  ...
  nb::class_<RobotModel>(m, "RobotModel")
      ...
      .def_prop_ro(
          "dof",
          [](const RobotModel &r) { return r.getActuatedJoints().size(); })
      .def("get_joint_names",
           [](const RobotModel &r) {
  ...
  nb::class_<URDFParser>(m, "URDFParser")
      ...
      .def_prop_rw("base_directory", &URDFParser::getBaseDirectory,
                   &URDFParser::setBaseDirectory);
  ...
  nb::class_<ForwardKinematics>(m, "ForwardKinematics")
      .def(nb::init<std::shared_ptr<const RobotModel>, const std::string &,
                    const std::string &>(),
           nb::arg("robot"), nb::arg("end_link"), nb::arg("base_link") = "")
      .def("compute", &ForwardKinematics::compute, nb::arg("joint_angles"),
           nb::arg("check_bounds") = false)
      .def("compute_to_link", &ForwardKinematics::computeToLink,
           nb::arg("joint_angles"), nb::arg("target_link"),
           nb::arg("check_bounds") = false)
      .def("compute_all_link_transforms",
           &ForwardKinematics::computeAllLinkTransforms,
  ...
  nb::class_<JacobianCalculator>(m, "JacobianCalculator")
      .def(nb::init<std::shared_ptr<const RobotModel>, const std::string &,
                    const std::string &>(),
           nb::arg("robot"), nb::arg("end_link"), nb::arg("base_link") = "")
      .def("compute", &JacobianCalculator::compute, nb::arg("joint_angles"),
           nb::arg("type") = JacobianType::Analytic,
           nb::arg("target_link") = "")
      .def("compute_jacobian_derivative",
           &JacobianCalculator::computeJacobianDerivative,
      ...
      .def_static("convert_jacobian", &JacobianCalculator::convertJacobian,
                  nb::arg("jacobian"), nb::arg("pose"), nb::arg("from_type"),
                  nb::arg("to_type"));
  ...
  nb::class_<SolverConfig>(m, "SolverConfig")
      ...
      .def_rw("orientation_anchor_weight",
  ...
  nb::class_<SolverStatus>(m, "SolverStatus")
      ...
      .def("__repr__", [](const SolverStatus &s) {
        return "SolverStatus(converged=" +
               std::string(s.converged ? "True" : "False") +
  ...
  nb::class_<IKResult>(m, "IKResult")
      ...
      .def("__repr__", [](const IKResult &r) {
        return "IKResult(status=" +
               std::string(r.status.converged ? "Converged" : "Failed") + ")";
      });
  ...
  nb::class_<SQPIKSolver, IKSolver>(m, "SQPIKSolver")
      ...
      .def(
          "solve",
          [](SQPIKSolver &self, const Transform &target_pose,
             const Eigen::VectorXd &initial_guess) {
            Eigen::VectorXd solution;
            SolverStatus status =
                self.solve(target_pose, initial_guess, solution);
  ...
  nb::class_<Robot>(m, "Robot")
      ...
      .def_static("from_urdf_string", &Robot::fromURDFString,
                  nb::arg("urdf_string"), nb::arg("end_link"),
                  nb::arg("base_link") = "",
                  nb::call_guard<nb::gil_scoped_release>())
      .def("clone", &Robot::clone)
      .def("forward_kinematics", &Robot::forwardKinematics, nb::arg("q"),
           nb::arg("link") = "")
      .def("compute_pose", &Robot::computePose, nb::arg("q"),
           nb::arg("link") = "")
      .def(
          "inverse_kinematics",
          [](Robot &self, const Transform &target,
            ...
            auto [solution, status] =
                self.inverseKinematics(target, q_init, link);
      ...
      .def("get_manipulability", &Robot::getManipulability, nb::arg("q"),
           nb::arg("link") = "")
      .def("is_singular", &Robot::isSingular, nb::arg("q"),
           nb::arg("threshold") = 1e-6, nb::arg("link") = "")
      .def("get_condition_number", &Robot::getConditionNumber, nb::arg("q"),
           nb::arg("link") = "")
      .def("set_ik_tolerance", &Robot::setIKTolerance, nb::arg("tol"))
      .def("set_position_only_ik", &Robot::setPositionOnlyIK, nb::arg("enable"))
      .def("set_orientation_only_ik", &Robot::setOrientationOnlyIK,
           nb::arg("enable"))
