...
The current CppAD-based Jacobian computation is a performance bottleneck in the inverse kinematics solver. Automatic differentiation introduces overhead from tape recording/playback that is unnecessary for rigid body kinematics, where the kinematic structure is known a priori. By replacing CppAD with analytical geometric methods, we can achieve a **5-10x performance improvement** (from ~50µs to <5µs per Jacobian computation) while simplifying the codebase, reducing dependencies, and improving maintainability.
