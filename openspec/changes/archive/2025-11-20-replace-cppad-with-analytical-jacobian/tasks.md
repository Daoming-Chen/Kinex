...
- [x] Implement Jacobian column computation
  - Linear part: `z_i Ã— (p_ee - p_i)` using `Eigen::Vector3d::cross()`
  - Angular part: `z_i` directly
  - Fill 6Ã—n output matrix column-by-column
...
- [ ] Add `FiniteDifferenceValidation` test
  ...
  - Compute numerical Jacobian via central difference (Îµ = 1e-7)
  - Assert error < 1e-4
...
- [x] Add micro-benchmark for Jacobian computation
  ...
  - Target: <5Âµs for 7-DOF arm on i7-12700K
