...
For a serial manipulator with joint configuration `q ‚à?‚Ñù‚Åø`, the spatial Jacobian `J ‚à?‚Ñù‚Å∂À£‚Åø` relates joint velocities to end-effector twist:

```
·∫?= J(q) qÃá

where ·∫?= [v; œâ] ‚à?‚Ñù‚Å∂  (linear and angular velocity in world frame)
```
...
```
Input: Joint angles q ‚à?‚Ñù‚Åø
Output: Jacobian J ‚à?‚Ñù‚Å∂À£‚Å?
...
4. For each joint i = 0..n-1:
   a. Compute cross product: z_i √ó (p_ee - p_i)
   b. Fill column: J.col(i) = [cross_result; z_i]
```

**Complexity:**
- Time: O(n) ‚Ä?single forward pass + O(n) cross products
- Space: O(n) ‚Ä?cache n joint frames (3 vectors each: z_i, p_i, p_ee)
...
- ‚ú?Create `AnalyticalJacobianCalculator` in new files:
  - `core/include/kinex/analytical_jacobian.h`
  - `core/src/analytical_jacobian.cpp`
- ‚ú?Implement core algorithm (forward pass + Jacobian fill)
- ‚ú?Add unit tests for accuracy validation

### Phase 2: API Compatibility (Week 1-2)
- ‚ú?Port all methods from `JacobianCalculator`:
  - `compute()`, `isSingular()`, `getManipulability()`, etc.
- ‚ú?Ensure identical results (within numerical tolerance)
- ‚ú?Add integration tests with `SQPIKSolver`

### Phase 3: Integration (Week 2)
- ‚ú?Update `SQPIKSolver` to use `AnalyticalJacobianCalculator`
- ‚ú?Run full test suite (all tests pass)
- ‚ú?Benchmark and document performance improvements

### Phase 4: Cleanup (Week 2)
- ‚ú?Remove `JacobianCalculator` class
- ‚ú?Remove CppAD includes from `kinematics.cpp` and headers
- ‚ú?Update CMakeLists.txt to remove CppAD dependency
- ‚ú?Remove `third_party/CppAD` submodule
- ‚ú?Update documentation and README
...
|---------------|-----------------|---------------------|---------|
| UR5e (6-DOF)  | 20-30¬µs         | 3-5¬µs               | 5-7x    |
| Panda (7-DOF) | 25-35¬µs         | 4-6¬µs               | 5-7x    |
| UR10 (6-DOF)  | 20-30¬µs         | 3-5¬µs               | 5-7x    |

### End-to-End IK Benchmarks

| Scenario      | Current (CppAD) | Target (Analytical) | Speedup |
|---------------|-----------------|---------------------|---------|
| Cold Start    | 150-250¬µs       | 60-100¬µs            | 2-3x    |
| Warm Start    | 100-180¬µs       | 40-70¬µs             | 2-3x    |
| Trajectory    | 120-200¬µs       | 50-80¬µs             | 2-3x    |
